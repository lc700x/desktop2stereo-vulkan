...
- Glow (OpenXR): GPU source was gated to `backend_name == "cuda"` (forced cpu_fallback on ROCm) and the glow backend hardcoded CudaVulkanImageImporter. Fixed: allow rocm/hip in the gate; shared `_create_interop_importer()` picks RocmVulkanImageImporter; added ROCm `register_buffer` + `copy_tensor_to_buffer` (hipExternalMemoryGetMappedBuffer + hipMemcpyAsync). Then `vkQueueSubmit` on the glow **compute** queue returned VK_ERROR_UNKNOWN when waiting on the HIP-signaled external binary semaphore (AMD LLPC; the graphics-queue local-viewer path is unaffected). Fix: `hipStreamSynchronize` after the buffer copy and submit **without** the semaphore wait. Glow GPU path error-free; OpenXR runtime ~60 FPS (was ~4-9 with the per-frame error).
