...
- Glow (OpenXR): fix chain — gate allowed rocm; `_create_interop_importer()` picks RocmVulkanImageImporter; ROCm `register_buffer`+`copy_tensor_to_buffer`; then `vkQueueSubmit` on the glow **compute** queue returned VK_ERROR_UNKNOWN when waiting on the HIP-signaled external binary semaphore (AMD LLPC). Fix: submit without the semaphore wait and use a **synchronous hipMemcpy** for the buffer copy (no hipStreamSynchronize, which could hang in the pipeline thread and stall the OpenXR compositor). Glow GPU path error-free; OpenXR runtime ~60 FPS.
- **GUI capture tool on AMD**: `get_capture_tool_options` keyed the vendor branch on the label containing `"CUDA"`; the build path passes the resolved capture-tool name (`"WindowsCaptureROCm"` — no `"CUDA"` substring), so on AMD it fell back to `WindowsCapture` and the saved `WindowsCaptureROCm` never loaded → CPU capture (low SBS / freeze). Fix: branch on `devices_module.IS_ROCM` first. GUI must run on the **ROCm env** (`src\python3`) for `IS_ROCM` to be True (`devices.get_devices` sets it from `torch.version.hip`). To get the GPU path: select `Capture Tool → WindowsCaptureROCm` in the GUI (now offered on AMD) and save.
