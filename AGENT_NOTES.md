...
- Glow (OpenXR): **GPU glow is DISABLED on ROCm (cpu_fallback)** for OpenXR stability. The GPU glow path (`VulkanGlowSourceComputeBackend`, plus the HIP buffer importer work) on AMD LLPC destabilized the OpenXR compositor — the headset went idle and the compositor's present dropped to ~9 FPS (screen stopped updating). Reverting to the cpu_fallback glow on ROCm restored the OpenXR compositor to ~73 FPS present with a 54-60 FPS pipeline. (Investigation notes: the glow compute `vkQueueSubmit` on the compute queue returned VK_ERROR_UNKNOWN when waiting on the HIP-signaled binary semaphore; and the glow gate + `RocmVulkanImageImporter` + `register_buffer`/`copy_tensor_to_buffer` were added to enable it — all now gated behind `backend_name == "cuda"` again for ROCm.)
